...
@Table(
        ...
        uniqueConstraints = @UniqueConstraint(
                name = "uk_whouse_company_code",
                columnNames = {"company_id", "code"}
        ),
        indexes = {
                @Index(name = "idx_whouse_company", columnList = "company_id"),
                @Index(name = "idx_whouse_branch", columnList = "branch_id")
        }
)
public class Whouse {
    ...
    /** Multitenancy discriminator (όπως στο Branch) */
    @TenantId
    @Column(name = "company_id", nullable = false, updatable = false)
    private Long companyId;

    /** Μοναδικός κωδικός μέσα στην εταιρεία (π.χ. ATH-W1) */
    ...
    // Διεύθυνση / Επικοινωνία (όπως branch)
    ...
    /** Αν θες να το χρησιμοποιήσεις σαν "κύρια αποθήκη" */
    ...
    /** Σύνδεση με υποκατάστημα */
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(
            name = "branch_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_whouse_branch")
    )
